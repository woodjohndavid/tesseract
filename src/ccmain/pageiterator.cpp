...
bool PageIterator::BoundingBoxInternal(PageIteratorLevel level, int *left,
  ...
  switch (level) {
    ...
    case RIL_WORD:
      box = it_->word()->word->restricted_bounding_box(include_upper_dots_,
                                                       include_lower_dots_);
      debug_str = "word level box ";               // JDWDEBUG
      box.print_to_str(debug_str);                 // JDWDEBUG
      fprintf(stderr, "%s \n", debug_str.c_str()); // JDWDEBUG
      break;
    case RIL_SYMBOL:
      if (cblob_it_ == nullptr) {
        box = it_->word()->box_word->BlobBox(blob_index_);
        debug_str = "cblob null box ";               // JDWDEBUG
        box.print_to_str(debug_str);                 // JDWDEBUG
      } else {
        box = cblob_it_->data()->bounding_box();
        debug_str = "cblob not null box ";           // JDWDEBUG
        box.print_to_str(debug_str);                 // JDWDEBUG
      }
  }
  if (level == RIL_PARA) {
  ...
  *bottom = ClipToRange(pix_height - box.bottom(), *top, pix_height);
  ...
  if (level == RIL_SYMBOL) {
    ...
    debug_str += "," + std::to_string(*top);
    debug_str += ")->(" + std::to_string(*right);
    debug_str += "," + std::to_string(*bottom);
    debug_str += ')';
    fprintf(stderr, "%s \n", debug_str.c_str());
  }
  // JDWDEBUG END
